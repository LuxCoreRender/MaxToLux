...
#define LUX_CLAMP_CLASS_ID	Class_ID(0x7d03463, 0x4f3489d9)
...
class Lux_ClampSampler : public MapSampler
{
private:
	Lux_Clamp* mTexture;
public:
	Lux_ClampSampler() : mTexture(nullptr) { }
	Lux_ClampSampler(Lux_Clamp *c) { mTexture = c; }
	~Lux_ClampSampler() { }

	void   Set(Lux_Clamp *c) { mTexture = c; }
...
public:
	...
	//From Animatable
	virtual Class_ID  ClassID() { return LUX_CLAMP_CLASS_ID; }
	virtual SClass_ID SuperClassID() { return TEXMAP_CLASS_ID; }
	virtual void GetClassName(TSTR& s, bool localized) { s = GetString(IDS_CLASS_CLAMP); }
...
class Lux_ClampClassDesc : public ClassDesc2
...
public:
	...
	virtual void* Create(BOOL /*loading = FALSE*/) { return new Lux_Clamp(); }
	virtual const TCHAR *	ClassName() { return GetString(IDS_CLASS_CLAMP); }
	const TCHAR*  NonLocalizedClassName() { return GetString(IDS_CLASS_CLAMP); }
	virtual SClass_ID SuperClassID() { return TEXMAP_CLASS_ID; }
	virtual Class_ID ClassID() { return LUX_CLAMP_CLASS_ID; }
	...
	virtual const TCHAR* InternalName() { return _T("Lux_Clamp"); }	// returns fixed parsable name (scripter-visible name)
	virtual HINSTANCE HInstance() { return hInstance; }					// returns owning module handle

};


ClassDesc2* GetLux_ClampDesc() {
	static Lux_ClampClassDesc Lux_ClampDesc;
	return &Lux_ClampDesc;
}


enum { Lux_Clamp_params };
...
static ParamBlockDesc2 Lux_Clamp_param_blk(Lux_Clamp_params, _T("params"), 0, GetLux_ClampDesc(),
	P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	//rollout
	IDD_CLAMP_PANEL, IDS_CLAMP_PARAMS, 0, 0, NULL,
	// params
	value1, _T("clamp value 1"), TYPE_FLOAT, P_ANIMATABLE, IDS_CLAMP_VALUE,
	p_default, 1.0f,
	p_range, 0.001f, 1.0f,
	p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_CLAMP_VALUE, IDC_CLAMP_VALUE_SPIN, 0.001f,
	p_end,

	texture1_map, _T("clamp texture 1 map"), TYPE_TEXMAP, P_OWNERS_REF, IDS_CLAMP_TEXTURE,
	p_refno, TEXTURE1_REF,
	p_subtexno, 0,
	p_ui, TYPE_TEXMAPBUTTON, IDC_CLAMP_TEXTURE,
	p_end,

	min, _T("clamp min"), TYPE_FLOAT, P_ANIMATABLE, IDS_CLAMP_MIN,
	p_default, 1.0f,
	p_range, 0.001f, 1.0f,
	p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_CLAMP_MIN, IDC_CLAMP_MIN_SPIN, 0.001f,
	p_end,

	max, _T("clamp max"), TYPE_FLOAT, P_ANIMATABLE, IDS_CLAMP_MAX,
	p_default, 1.0f,
	p_range, 0.001f, 1.0f,
	p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_CLAMP_MAX, IDC_CLAMP_MAX_SPIN, 0.001f,
	p_end,
	...
	: pblock(nullptr)
{
	...
	GetLux_ClampDesc()->MakeAutoParamBlocks(this);
	Reset();
}
...
void Lux_Clamp::Update(TimeValue /*t*/, Interval& /*valid*/)
{
	//TODO: Add code to evaluate anything prior to rendering
}

Interval Lux_Clamp::Validity(TimeValue /*t*/)
{
	//TODO: Update ivalid here
	return ivalid;
}

ParamDlg* Lux_Clamp::CreateParamDlg(HWND hwMtlEdit, IMtlParams* imp)
{
	IAutoMParamDlg* masterDlg = GetLux_ClampDesc()->CreateParamDlgs(hwMtlEdit, imp, this);
...
BOOL Lux_Clamp::SetDlgThing(ParamDlg* dlg)
{
	if (dlg == uvGenDlg)
...
void Lux_Clamp::SetSubTexmap(int i, Texmap* m)
{
	ReplaceReference(i + 2, m);
	//TODO Store the 'i-th' sub-texmap managed by the texture
}

TSTR Lux_Clamp::GetSubTexmapSlotName(int i, bool localized)
{
...
//From ReferenceMaker
RefTargetHandle Lux_Clamp::GetReference(int i)
{
...
void Lux_Clamp::SetReference(int i, RefTargetHandle rtarg)
{
...
RefResult Lux_Clamp::NotifyRefChanged(const Interval& /*changeInt*/, RefTargetHandle hTarget, PartID& /*partID*/, RefMessage message, BOOL /*propagate*/)
{
	switch (message)
	{
...
RefTargetHandle Lux_Clamp::Clone(RemapDir &remap)
{
	Lux_Clamp *mnew = new Lux_Clamp();
...
Animatable* Lux_Clamp::SubAnim(int i)
{
...
TSTR Lux_Clamp::SubAnimName(int i, bool localized)
{
...
IOResult Lux_Clamp::Save(ISave* /*isave*/)
{
	//TODO: Add code to allow plug-in to save its data
	return IO_OK;
}

IOResult Lux_Clamp::Load(ILoad* /*iload*/)
{
	//TODO: Add code to allow plug-in to load its data
	return IO_OK;
}

AColor Lux_Clamp::EvalColor(ShadeContext& /*sc*/)
{
