...
#define LUX_POWER_CLASS_ID	Class_ID(0x7d03463, 0x4f4864d8)
...
class Lux_PowerSampler : public MapSampler
{
private:
	Lux_Power* mTexture;
public:
	Lux_PowerSampler() : mTexture(nullptr) { }
	Lux_PowerSampler(Lux_Power *c) { mTexture = c; }
	~Lux_PowerSampler() { }

	void   Set(Lux_Power *c) { mTexture = c; }
...
class Lux_PowerClassDesc : public ClassDesc2
...
public:
	...
	virtual const TCHAR* InternalName() { return _T("Lux_Power"); }	// returns fixed parsable name (scripter-visible name)
	virtual HINSTANCE HInstance() { return hInstance; }					// returns owning module handle

};


ClassDesc2* GetLux_PowerDesc() {
	static Lux_PowerClassDesc Lux_PowerDesc;
	return &Lux_PowerDesc;
}


enum { Lux_Power_params };
...
static ParamBlockDesc2 Lux_Power_param_blk(Lux_Power_params, _T("params"), 0, GetLux_PowerDesc(),
	...
	: pblock(nullptr)
{
	...
	GetLux_PowerDesc()->MakeAutoParamBlocks(this);
	Reset();
}
...
void Lux_Power::Update(TimeValue /*t*/, Interval& /*valid*/)
{
	//TODO: Add code to evaluate anything prior to rendering
}

Interval Lux_Power::Validity(TimeValue /*t*/)
{
	//TODO: Update ivalid here
	return ivalid;
}

ParamDlg* Lux_Power::CreateParamDlg(HWND hwMtlEdit, IMtlParams* imp)
{
	IAutoMParamDlg* masterDlg = GetLux_PowerDesc()->CreateParamDlgs(hwMtlEdit, imp, this);
...
BOOL Lux_Power::SetDlgThing(ParamDlg* dlg)
{
	if (dlg == uvGenDlg)
...
void Lux_Power::SetSubTexmap(int i, Texmap* m)
{
...
void Lux_Power::SetReference(int i, RefTargetHandle rtarg)
{
...
RefResult Lux_Power::NotifyRefChanged(const Interval& /*changeInt*/, RefTargetHandle hTarget, PartID& /*partID*/, RefMessage message, BOOL /*propagate*/)
{
	switch (message)
	{
...
RefTargetHandle Lux_Power::Clone(RemapDir &remap)
{
	Lux_Power *mnew = new Lux_Power();
...
Animatable* Lux_Power::SubAnim(int i)
{
...
IOResult Lux_Power::Save(ISave* /*isave*/)
{
	//TODO: Add code to allow plug-in to save its data
	return IO_OK;
}

IOResult Lux_Power::Load(ILoad* /*iload*/)
{
	//TODO: Add code to allow plug-in to load its data
	return IO_OK;
}

AColor Lux_Power::EvalColor(ShadeContext& /*sc*/)
{
