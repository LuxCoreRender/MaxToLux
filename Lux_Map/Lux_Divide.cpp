...
#define LUX_DIVIDE_CLASS_ID	Class_ID(0x7d03463, 0x4f6863d4)
...
class Lux_DivideSampler : public MapSampler
{
private:
	Lux_Divide* mTexture;
public:
	Lux_DivideSampler() : mTexture(nullptr) { }
	Lux_DivideSampler(Lux_Divide *c) { mTexture = c; }
	~Lux_DivideSampler() { }

	void   Set(Lux_Divide *c) { mTexture = c; }
...
class Lux_Divide : public Texmap {
public:
	...
	//From Animatable
	virtual Class_ID  ClassID() { return LUX_DIVIDE_CLASS_ID; }
...
class Lux_DivideClassDesc : public ClassDesc2
...
public:
	...
	virtual void* Create(BOOL /*loading = FALSE*/) { return new Lux_Divide(); }
	virtual const TCHAR *	ClassName() { return GetString(IDS_CLASS_DIVIDE); }
	const TCHAR*  NonLocalizedClassName() { return GetString(IDS_CLASS_DIVIDE); }
	virtual SClass_ID SuperClassID() { return TEXMAP_CLASS_ID; }
	virtual Class_ID ClassID() { return LUX_DIVIDE_CLASS_ID; }
	...
	virtual const TCHAR* InternalName() { return _T("Lux_Divide"); }	// returns fixed parsable name (scripter-visible name)
	virtual HINSTANCE HInstance() { return hInstance; }					// returns owning module handle

};


ClassDesc2* GetLux_DivideDesc() {
	static Lux_DivideClassDesc Lux_DivideDesc;
	return &Lux_DivideDesc;
}


enum { Lux_Divide_params };
...
static ParamBlockDesc2 Lux_Divide_param_blk(Lux_Divide_params, _T("params"), 0, GetLux_DivideDesc(),
	P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	//rollout
	IDD_DIVIDE_PANEL, IDS_DIVIDE_PARAMS, 0, 0, NULL,
	// params
	value1, _T("divide value 1"), TYPE_FLOAT, P_ANIMATABLE, IDS_MIX_VALUE_1,
	p_default, 1.0f,
	p_range, 0.001f, 1.0f,
	p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_DIVIDE_VALUE_1, IDC_DIVIDE_VALUE_1_SPIN, 0.001f,
	p_end,

	texture1_map, _T("divide texture 1 map"), TYPE_TEXMAP, P_OWNERS_REF, IDS_CHECKER_TEXTURE1_MAP,
	p_refno, TEXTURE1_REF,
	p_subtexno, 0,
	p_ui, TYPE_TEXMAPBUTTON, IDC_DIVIDE_TEXTURE1_MAP,
	p_end,

	value2, _T("divide value 2"), TYPE_FLOAT, P_ANIMATABLE, IDS_MIX_VALUE_1,
	p_default, 1.0f,
	p_range, 0.001f, 1.0f,
	p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_DIVIDE_VALUE_2, IDC_DIVIDE_VALUE_2_SPIN, 0.001f,
	p_end,

	texture2_map, _T("divide texture 2 map"), TYPE_TEXMAP, P_OWNERS_REF, IDS_CHECKER_TEXTURE2_MAP,
	p_refno, TEXTURE2_REF,
	p_subtexno, 1,
	p_ui, TYPE_TEXMAPBUTTON, IDC_DIVIDE_TEXTURE2_MAP,
	p_end,
...
ParamDlg* Lux_Divide::uvGenDlg;

//--- Lux_Divide -------------------------------------------------------
Lux_Divide::Lux_Divide()
	: pblock(nullptr)
{
	for (int i = 0; i < NSUBTEX; i++)
		subtex[i] = nullptr;
	//TODO: Add all the initializing stuff
	GetLux_DivideDesc()->MakeAutoParamBlocks(this);
	Reset();
}
...
void Lux_Divide::Update(TimeValue /*t*/, Interval& /*valid*/)
{
	//TODO: Add code to evaluate anything prior to rendering
}

Interval Lux_Divide::Validity(TimeValue /*t*/)
{
	//TODO: Update ivalid here
	return ivalid;
}

ParamDlg* Lux_Divide::CreateParamDlg(HWND hwMtlEdit, IMtlParams* imp)
{
	IAutoMParamDlg* masterDlg = GetLux_DivideDesc()->CreateParamDlgs(hwMtlEdit, imp, this);
...
BOOL Lux_Divide::SetDlgThing(ParamDlg* dlg)
{
	if (dlg == uvGenDlg)
...
void Lux_Divide::SetSubTexmap(int i, Texmap* m)
{
	ReplaceReference(i + 2, m);
	//TODO Store the 'i-th' sub-texmap managed by the texture
}

TSTR Lux_Divide::GetSubTexmapSlotName(int i, bool localized)
{
...
//From ReferenceMaker
RefTargetHandle Lux_Divide::GetReference(int i)
{
...
void Lux_Divide::SetReference(int i, RefTargetHandle rtarg)
{
...
RefResult Lux_Divide::NotifyRefChanged(const Interval& /*changeInt*/, RefTargetHandle hTarget, PartID& /*partID*/, RefMessage message, BOOL /*propagate*/)
{
	switch (message)
	{
...
RefTargetHandle Lux_Divide::Clone(RemapDir &remap)
{
	Lux_Divide *mnew = new Lux_Divide();
...
Animatable* Lux_Divide::SubAnim(int i)
{
...
TSTR Lux_Divide::SubAnimName(int i, bool localized)
{
...
IOResult Lux_Divide::Save(ISave* /*isave*/)
{
	//TODO: Add code to allow plug-in to save its data
	return IO_OK;
}

IOResult Lux_Divide::Load(ILoad* /*iload*/)
{
	//TODO: Add code to allow plug-in to load its data
	return IO_OK;
}

AColor Lux_Divide::EvalColor(ShadeContext& /*sc*/)
{
...
float Lux_Divide::EvalMono(ShadeContext& sc)
{
	//TODO: Evaluate the map for a "mono" channel
	return Intens(EvalColor(sc));
}

Point3 Lux_Divide::EvalNormalPerturb(ShadeContext& /*sc*/)
{
	//TODO: Return the perturbation to apply to a normal for bump mapping
	return Point3(0, 0, 0);
}

ULONG Lux_Divide::LocalRequirements(int subMtlNum)
{
	//TODO: Specify various requirements for the material
	return uvGen->Requirements(subMtlNum);
}

void Lux_Divide::ActivateTexDisplay(BOOL /*onoff*/)
{
	//TODO: Implement this only if SupportTexDisplay() returns TRUE
}

DWORD_PTR Lux_Divide::GetActiveTexHandle(TimeValue /*t*/, TexHandleMaker& /*maker*/)
{
