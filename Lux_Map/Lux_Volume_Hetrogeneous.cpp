...
#define LUX_VOLUME_HETRO_CLASS_ID	Class_ID(0x3588120a, 0x6598079e)
...
class Lux_Volume_Hetro;

namespace
{
	const wchar_t* LuxMixMapFriendlyClassName = L"Lux Volumetric Hetrogeneous Map";
}

class Lux_Volume_Hetro_Sampler : public MapSampler
{
private:
	Lux_Volume_Hetro* mTexture;
public:
	Lux_Volume_Hetro_Sampler() : mTexture(nullptr) { }
	Lux_Volume_Hetro_Sampler(Lux_Volume_Hetro *c) { mTexture = c; }
	~Lux_Volume_Hetro_Sampler() { }

	void   Set(Lux_Volume_Hetro *c) { mTexture = c; }
...
class Lux_Volume_Hetro : public Texmap {
public:
	//Constructor/Destructor
	Lux_Volume_Hetro();
	virtual ~Lux_Volume_Hetro();
	...
	//From Animatable
	virtual Class_ID  ClassID() { return LUX_VOLUME_HETRO_CLASS_ID; }
	virtual SClass_ID SuperClassID() { return TEXMAP_CLASS_ID; }
	virtual void GetClassName(TSTR& s, bool localized) { s = GetString(IDS_CLASS_VOLUME_HETRO); }
...
class Lux_Volume_HetroClassDesc : public ClassDesc2, public IMtlRender_Compatibility_MtlBase
...
public:
	...
	virtual void* Create(BOOL /*loading = FALSE*/) { return new Lux_Volume_Hetro(); }
	virtual const TCHAR *	ClassName() { return GetString(IDS_CLASS_VOLUME_HETRO); }
	const TCHAR*  NonLocalizedClassName() { return GetString(IDS_CLASS_VOLUME_HETRO); }
	virtual SClass_ID SuperClassID() { return TEXMAP_CLASS_ID; }
	virtual Class_ID ClassID() { return LUX_VOLUME_HETRO_CLASS_ID; }
	...
	virtual const TCHAR* InternalName() { return _T("Lux_Volume_Hetro"); }	// returns fixed parsable name (scripter-visible name)
...
bool Lux_Volume_HetroClassDesc::IsCompatibleWithRenderer(ClassDesc& renderer_class_desc)
{
	// Before 3ds Max 2017, Class_ID::operator==() returned an int.
	return true;
}

ClassDesc2* GetLux_Volume_HetroDesc() {
	static Lux_Volume_HetroClassDesc Lux_Volume_HetroDesc;
	return &Lux_Volume_HetroDesc;
}


enum { Lux_Volume_Hetro_params };

enum { Common_Param, Homo_Param, Hetro_Param };
...
static ParamBlockDesc2 Lux_Volume_Hetro_param_blk(Lux_Volume_Hetro_params, _T("params"), 0, GetLux_Volume_HetroDesc(),
	...
	Hetro_Param,	IDD_VOLUME_HETEROGENEOUS_PANEL,	IDS_VOLUME_HETRO_PARAM,		0, 0, NULL,
	...
	// Hetro
	hetro_step_size, _T("volume step size"), TYPE_FLOAT, P_ANIMATABLE, IDS_VOLUME_HETRO_STEPSIZE,
		p_default, 0.001f,
		p_range, 0.001f, 1.0f,
		p_ui, Hetro_Param, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_VOLUME_HETRO_STEPSIZE, IDC_VOLUME_HETRO_STEPSIZE_SPIN, 0.001f,
		p_end,

	hetro_max_step, _T("volume max step"), TYPE_FLOAT, P_ANIMATABLE, IDS_VOLUME_HETRO_MAX_STEP,
		p_default, 0.001f,
		p_range, 0.001f, 1.0f,
		p_ui, Hetro_Param, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_VOLUME_HETRO_MAX_STEP, IDC_VOLUME_HETRO_MAX_STEP_SPIN, 0.001f,
		p_end,

	hetro_priority, _T("volume hetro priority"), TYPE_INT, P_ANIMATABLE, IDS_VOLUME_HETRO_PRIORITY,
		p_default, 1,
		p_range, 1, 1000,
		p_ui, Hetro_Param, TYPE_SPINNER, EDITTYPE_INT, IDC_VOLUME_HETRO_PRIORITY, IDC_VOLUME_HETRO_PRIORITY_SPIN, 1,
		p_end,

	hetro_auto_step, _T("volume hetro auto step"), TYPE_BOOL, 0, IDS_VOLUME_HETRO_AUTO_STEP,
		p_default, TRUE,
		p_ui, Hetro_Param, TYPE_SINGLECHEKBOX, IDC_VOLUME_HETRO_AUTO_STEP,
		p_end,
...
ParamDlg* Lux_Volume_Hetro::uvGenDlg;

//--- Lux_Volume_Hetro -------------------------------------------------------
Lux_Volume_Hetro::Lux_Volume_Hetro()
	: pblock(nullptr)
{
	for (int i = 0; i < NSUBTEX; i++)
		subtex[i] = nullptr;
	//TODO: Add all the initializing stuff
	GetLux_Volume_HetroDesc()->MakeAutoParamBlocks(this);
	Reset();
}
...
void Lux_Volume_Hetro::Update(TimeValue /*t*/, Interval& /*valid*/)
{
	//TODO: Add code to evaluate anything prior to rendering
}

Interval Lux_Volume_Hetro::Validity(TimeValue /*t*/)
{
	//TODO: Update ivalid here
	return ivalid;
}

ParamDlg* Lux_Volume_Hetro::CreateParamDlg(HWND hwMtlEdit, IMtlParams* imp)
{
	IAutoMParamDlg* masterDlg = GetLux_Volume_HetroDesc()->CreateParamDlgs(hwMtlEdit, imp, this);
...
BOOL Lux_Volume_Hetro::SetDlgThing(ParamDlg* dlg)
{
	if (dlg == uvGenDlg)
...
void Lux_Volume_Hetro::SetSubTexmap(int i, Texmap* m)
{
	ReplaceReference(i + 2, m);
	//TODO Store the 'i-th' sub-texmap managed by the texture
}

TSTR Lux_Volume_Hetro::GetSubTexmapSlotName(int i, bool localized)
{
...
//From ReferenceMaker
RefTargetHandle Lux_Volume_Hetro::GetReference(int i)
{
...
void Lux_Volume_Hetro::SetReference(int i, RefTargetHandle rtarg)
{
...
RefResult Lux_Volume_Hetro::NotifyRefChanged(const Interval& /*changeInt*/, RefTargetHandle hTarget, PartID& /*partID*/, RefMessage message, BOOL /*propagate*/)
{
	switch (message)
	{
...
RefTargetHandle Lux_Volume_Hetro::Clone(RemapDir &remap)
{
	Lux_Volume_Hetro *mnew = new Lux_Volume_Hetro();
...
Animatable* Lux_Volume_Hetro::SubAnim(int i)
{
...
TSTR Lux_Volume_Hetro::SubAnimName(int i, bool localized)
{
...
IOResult Lux_Volume_Hetro::Save(ISave* /*isave*/)
{
	//TODO: Add code to allow plug-in to save its data
	return IO_OK;
}

IOResult Lux_Volume_Hetro::Load(ILoad* /*iload*/)
{
	//TODO: Add code to allow plug-in to load its data
	return IO_OK;
}

AColor Lux_Volume_Hetro::EvalColor(ShadeContext& /*sc*/)
{
...
float Lux_Volume_Hetro::EvalMono(ShadeContext& sc)
{
	//TODO: Evaluate the map for a "mono" channel
	return Intens(EvalColor(sc));
}

Point3 Lux_Volume_Hetro::EvalNormalPerturb(ShadeContext& /*sc*/)
{
	//TODO: Return the perturbation to apply to a normal for bump mapping
	return Point3(0, 0, 0);
}

ULONG Lux_Volume_Hetro::LocalRequirements(int subMtlNum)
{
	//TODO: Specify various requirements for the material
	return uvGen->Requirements(subMtlNum);
}

void Lux_Volume_Hetro::ActivateTexDisplay(BOOL /*onoff*/)
{
	//TODO: Implement this only if SupportTexDisplay() returns TRUE
}

DWORD_PTR Lux_Volume_Hetro::GetActiveTexHandle(TimeValue /*t*/, TexHandleMaker& /*maker*/)
{
