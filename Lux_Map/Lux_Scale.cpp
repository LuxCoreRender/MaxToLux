...
#define LUX_SCALE_CLASS_ID	Class_ID(0x7d03463, 0x4f3872d2)
...
class Lux_ScaleSampler : public MapSampler
{
private:
	Lux_Scale* mTexture;
public:
	Lux_ScaleSampler() : mTexture(nullptr) { }
	Lux_ScaleSampler(Lux_Scale *c) { mTexture = c; }
	~Lux_ScaleSampler() { }

	void   Set(Lux_Scale *c) { mTexture = c; }
...
public:
	...
	//From Animatable
	virtual Class_ID  ClassID() { return LUX_SCALE_CLASS_ID; }
	virtual SClass_ID SuperClassID() { return TEXMAP_CLASS_ID; }
	virtual void GetClassName(TSTR& s, bool localized) { s = GetString(IDS_CLASS_SCALE); }
...
class Lux_ScaleClassDesc : public ClassDesc2
...
public:
	...
	virtual void* Create(BOOL /*loading = FALSE*/) { return new Lux_Scale(); }
	virtual const TCHAR *	ClassName() { return GetString(IDS_CLASS_SCALE); }
	const TCHAR*  NonLocalizedClassName() { return GetString(IDS_CLASS_SCALE); }
	virtual SClass_ID SuperClassID() { return TEXMAP_CLASS_ID; }
	virtual Class_ID ClassID() { return LUX_SCALE_CLASS_ID; }
	virtual const TCHAR* Category() { return NULL; }//GetString(IDS_CATEGORY); }
	...
	virtual const TCHAR* InternalName() { return _T("Lux_Scale"); }	// returns fixed parsable name (scripter-visible name)
	virtual HINSTANCE HInstance() { return hInstance; }					// returns owning module handle

};


ClassDesc2* GetLux_ScaleDesc() {
	static Lux_ScaleClassDesc Lux_ScaleDesc;
	return &Lux_ScaleDesc;
}


enum { Lux_Scale_params };
...
static ParamBlockDesc2 Lux_Scale_param_blk(Lux_Scale_params, _T("params"), 0, GetLux_ScaleDesc(),
	P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	//rollout
	IDD_SCALE_PANEL, IDS_SCALE_PARAMS, 0, 0, NULL,
	// params
	value1, _T("scale value 1"), TYPE_FLOAT, P_ANIMATABLE, IDS_SCALE_VALUE_1,
	p_default, 1.0f,
	p_range, 0.001f, 1.0f,
	p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_SCALE_VALUE_1, IDC_SCALE_VALUE_1_SPIN, 0.001f,
	p_end,

	texture1_map, _T("scale texture 1 map"), TYPE_TEXMAP, P_OWNERS_REF, IDS_SCALE_TEXTURE_1,
	p_refno, TEXTURE1_REF,
	p_subtexno, 0,
	p_ui, TYPE_TEXMAPBUTTON, IDC_SCALE_TEXTURE_1,
	p_end,

	value2, _T("scale value 1"), TYPE_FLOAT, P_ANIMATABLE, IDS_MIX_VALUE_1,
	p_default, 1.0f,
	p_range, 0.001f, 1.0f,
	p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_SCALE_VALUE_2, IDC_SCALE_VALUE_2_SPIN, 0.001f,
	p_end,

	texture2_map, _T("scale texture 2 map"), TYPE_TEXMAP, P_OWNERS_REF, IDS_SCALE_TEXTURE_2,
	p_refno, TEXTURE2_REF,
	p_subtexno, 1,
	p_ui, TYPE_TEXMAPBUTTON, IDC_SCALE_TEXTURE_2,
	p_end,
	...
	: pblock(nullptr)
{
	...
	GetLux_ScaleDesc()->MakeAutoParamBlocks(this);
	Reset();
}
...
void Lux_Scale::Update(TimeValue /*t*/, Interval& /*valid*/)
{
	//TODO: Add code to evaluate anything prior to rendering
}

Interval Lux_Scale::Validity(TimeValue /*t*/)
{
	//TODO: Update ivalid here
	return ivalid;
}

ParamDlg* Lux_Scale::CreateParamDlg(HWND hwMtlEdit, IMtlParams* imp)
{
	IAutoMParamDlg* masterDlg = GetLux_ScaleDesc()->CreateParamDlgs(hwMtlEdit, imp, this);
...
BOOL Lux_Scale::SetDlgThing(ParamDlg* dlg)
{
	if (dlg == uvGenDlg)
...
void Lux_Scale::SetSubTexmap(int i, Texmap* m)
{
	ReplaceReference(i + 2, m);
	//TODO Store the 'i-th' sub-texmap managed by the texture
}

TSTR Lux_Scale::GetSubTexmapSlotName(int i, bool localized)
{
...
//From ReferenceMaker
RefTargetHandle Lux_Scale::GetReference(int i)
{
...
void Lux_Scale::SetReference(int i, RefTargetHandle rtarg)
{
...
RefResult Lux_Scale::NotifyRefChanged(const Interval& /*changeInt*/, RefTargetHandle hTarget, PartID& /*partID*/, RefMessage message, BOOL /*propagate*/)
{
	switch (message)
	{
...
RefTargetHandle Lux_Scale::Clone(RemapDir &remap)
{
	Lux_Scale *mnew = new Lux_Scale();
...
Animatable* Lux_Scale::SubAnim(int i)
{
...
TSTR Lux_Scale::SubAnimName(int i, bool localized)
{
...
IOResult Lux_Scale::Save(ISave* /*isave*/)
{
	//TODO: Add code to allow plug-in to save its data
	return IO_OK;
}

IOResult Lux_Scale::Load(ILoad* /*iload*/)
{
	//TODO: Add code to allow plug-in to load its data
	return IO_OK;
}

AColor Lux_Scale::EvalColor(ShadeContext& /*sc*/)
{
