...
#define LUX_DOTPRODUCT_CLASS_ID	Class_ID(0x7d03463, 0x4f6834d2)
...
class Lux_DotProduct;

class Lux_DotProductSampler : public MapSampler
{
private:
	Lux_DotProduct* mTexture;
public:
	Lux_DotProductSampler() : mTexture(nullptr) { }
	Lux_DotProductSampler(Lux_DotProduct *c) { mTexture = c; }
	~Lux_DotProductSampler() { }

	void   Set(Lux_DotProduct *c) { mTexture = c; }
...
class Lux_DotProduct : public Texmap {
public:
	//Constructor/Destructor
	Lux_DotProduct();
	virtual ~Lux_DotProduct();
...
class Lux_DotProductClassDesc : public ClassDesc2
...
public:
	...
	virtual const TCHAR* InternalName() { return _T("Lux_DotProduct"); }	// returns fixed parsable name (scripter-visible name)
	virtual HINSTANCE HInstance() { return hInstance; }					// returns owning module handle

};


ClassDesc2* GetLux_DotProductDesc() {
	static Lux_DotProductClassDesc Lux_DotProductDesc;
	return &Lux_DotProductDesc;
}


enum { Lux_DotProduct_params };
...
static ParamBlockDesc2 Lux_DotProduct_param_blk(Lux_DotProduct_params, _T("params"), 0, GetLux_DotProductDesc(),
...
ParamDlg* Lux_DotProduct::uvGenDlg;

//--- Lux_DotProduct -------------------------------------------------------
Lux_DotProduct::Lux_DotProduct()
	: pblock(nullptr)
{
	for (int i = 0; i < NSUBTEX; i++)
		subtex[i] = nullptr;
	//TODO: Add all the initializing stuff
	GetLux_DotProductDesc()->MakeAutoParamBlocks(this);
	Reset();
}
...
void Lux_DotProduct::Update(TimeValue /*t*/, Interval& /*valid*/)
{
	//TODO: Add code to evaluate anything prior to rendering
}

Interval Lux_DotProduct::Validity(TimeValue /*t*/)
{
	//TODO: Update ivalid here
	return ivalid;
}

ParamDlg* Lux_DotProduct::CreateParamDlg(HWND hwMtlEdit, IMtlParams* imp)
{
	IAutoMParamDlg* masterDlg = GetLux_DotProductDesc()->CreateParamDlgs(hwMtlEdit, imp, this);
...
BOOL Lux_DotProduct::SetDlgThing(ParamDlg* dlg)
{
	if (dlg == uvGenDlg)
...
void Lux_DotProduct::SetSubTexmap(int i, Texmap* m)
{
...
//From ReferenceMaker
RefTargetHandle Lux_DotProduct::GetReference(int i)
{
...
void Lux_DotProduct::SetReference(int i, RefTargetHandle rtarg)
{
...
RefResult Lux_DotProduct::NotifyRefChanged(const Interval& /*changeInt*/, RefTargetHandle hTarget, PartID& /*partID*/, RefMessage message, BOOL /*propagate*/)
{
	switch (message)
	{
...
RefTargetHandle Lux_DotProduct::Clone(RemapDir &remap)
{
	Lux_DotProduct *mnew = new Lux_DotProduct();
...
Animatable* Lux_DotProduct::SubAnim(int i)
{
...
IOResult Lux_DotProduct::Save(ISave* /*isave*/)
{
	//TODO: Add code to allow plug-in to save its data
	return IO_OK;
}

IOResult Lux_DotProduct::Load(ILoad* /*iload*/)
{
	//TODO: Add code to allow plug-in to load its data
	return IO_OK;
}

AColor Lux_DotProduct::EvalColor(ShadeContext& /*sc*/)
{
...
float Lux_DotProduct::EvalMono(ShadeContext& sc)
{
	//TODO: Evaluate the map for a "mono" channel
	return Intens(EvalColor(sc));
}

Point3 Lux_DotProduct::EvalNormalPerturb(ShadeContext& /*sc*/)
{
	//TODO: Return the perturbation to apply to a normal for bump mapping
	return Point3(0, 0, 0);
}

ULONG Lux_DotProduct::LocalRequirements(int subMtlNum)
{
	//TODO: Specify various requirements for the material
	return uvGen->Requirements(subMtlNum);
}

void Lux_DotProduct::ActivateTexDisplay(BOOL /*onoff*/)
{
	//TODO: Implement this only if SupportTexDisplay() returns TRUE
}

DWORD_PTR Lux_DotProduct::GetActiveTexHandle(TimeValue /*t*/, TexHandleMaker& /*maker*/)
{
