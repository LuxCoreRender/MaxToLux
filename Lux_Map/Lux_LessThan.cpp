...
#define LUX_LESSTHAN_CLASS_ID	Class_ID(0x7d03463, 0x4f8674d1)
...
class Lux_LessThanSampler : public MapSampler
{
private:
	Lux_LessThan* mTexture;
public:
	Lux_LessThanSampler() : mTexture(nullptr) { }
	Lux_LessThanSampler(Lux_LessThan *c) { mTexture = c; }
	~Lux_LessThanSampler() { }

	void   Set(Lux_LessThan *c) { mTexture = c; }
...
class Lux_LessThan : public Texmap {
public:
	//Constructor/Destructor
	Lux_LessThan();
	virtual ~Lux_LessThan();
	...
	//From Animatable
	virtual Class_ID  ClassID() { return LUX_LESSTHAN_CLASS_ID; }
	virtual SClass_ID SuperClassID() { return TEXMAP_CLASS_ID; }
	virtual void GetClassName(TSTR& s, bool localized) { s = GetString(IDS_CLASS_LESSTHAN); }
...
class Lux_LessThanClassDesc : public ClassDesc2
...
public:
	...
	virtual void* Create(BOOL /*loading = FALSE*/) { return new Lux_LessThan(); }
	virtual const TCHAR *	ClassName() { return GetString(IDS_CLASS_LESSTHAN); }
	const TCHAR*  NonLocalizedClassName() { return GetString(IDS_CLASS_LESSTHAN); }
	virtual SClass_ID SuperClassID() { return TEXMAP_CLASS_ID; }
	virtual Class_ID ClassID() { return LUX_LESSTHAN_CLASS_ID; }
	...
	virtual const TCHAR* InternalName() { return _T("Lux_LessThan"); }	// returns fixed parsable name (scripter-visible name)
	virtual HINSTANCE HInstance() { return hInstance; }					// returns owning module handle

};


ClassDesc2* GetLux_LessThanDesc() {
	static Lux_LessThanClassDesc Lux_LessThanDesc;
	return &Lux_LessThanDesc;
}


enum { Lux_LessThan_params };
...
static ParamBlockDesc2 Lux_LessThan_param_blk(Lux_LessThan_params, _T("params"), 0, GetLux_LessThanDesc(),
	P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	//rollout
	IDD_LESSTHAN_PANEL, IDS_LESSTHAN_PARAMS, 0, 0, NULL,
	// params
	value1, _T("lessthan value 1"), TYPE_FLOAT, P_ANIMATABLE, IDS_MIX_VALUE_1,
	p_default, 1.0f,
	p_range, 0.001f, 1.0f,
	p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_LESSTHAN_VALUE_1, IDC_LESSTHAN_VALUE_1_SPIN, 0.001f,
	p_end,

	texture1_map, _T("lessthan texture 1 map"), TYPE_TEXMAP, P_OWNERS_REF, IDS_CHECKER_TEXTURE1_MAP,
	p_refno, TEXTURE1_REF,
	p_subtexno, 0,
	p_ui, TYPE_TEXMAPBUTTON, IDC_LESSTHAN_TEXTURE1_MAP,
	p_end,

	value2, _T("lessthan value 2"), TYPE_FLOAT, P_ANIMATABLE, IDS_MIX_VALUE_1,
	p_default, 1.0f,
	p_range, 0.001f, 1.0f,
	p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_LESSTHAN_VALUE_2, IDC_LESSTHAN_VALUE_2_SPIN, 0.001f,
	p_end,

	texture2_map, _T("lessthan texture 2 map"), TYPE_TEXMAP, P_OWNERS_REF, IDS_CHECKER_TEXTURE2_MAP,
	p_refno, TEXTURE2_REF,
	p_subtexno, 1,
	p_ui, TYPE_TEXMAPBUTTON, IDC_LESSTHAN_TEXTURE2_MAP,
	p_end,
...
ParamDlg* Lux_LessThan::uvGenDlg;

//--- Lux_LessThan -------------------------------------------------------
Lux_LessThan::Lux_LessThan()
	: pblock(nullptr)
{
	for (int i = 0; i < NSUBTEX; i++)
		subtex[i] = nullptr;
	//TODO: Add all the initializing stuff
	GetLux_LessThanDesc()->MakeAutoParamBlocks(this);
	Reset();
}
...
void Lux_LessThan::Update(TimeValue /*t*/, Interval& /*valid*/)
{
	//TODO: Add code to evaluate anything prior to rendering
}

Interval Lux_LessThan::Validity(TimeValue /*t*/)
{
	//TODO: Update ivalid here
	return ivalid;
}

ParamDlg* Lux_LessThan::CreateParamDlg(HWND hwMtlEdit, IMtlParams* imp)
{
	IAutoMParamDlg* masterDlg = GetLux_LessThanDesc()->CreateParamDlgs(hwMtlEdit, imp, this);
...
BOOL Lux_LessThan::SetDlgThing(ParamDlg* dlg)
{
	if (dlg == uvGenDlg)
...
void Lux_LessThan::SetSubTexmap(int i, Texmap* m)
{
	ReplaceReference(i + 2, m);
	//TODO Store the 'i-th' sub-texmap managed by the texture
}

TSTR Lux_LessThan::GetSubTexmapSlotName(int i, bool localized)
{
...
//From ReferenceMaker
RefTargetHandle Lux_LessThan::GetReference(int i)
{
...
void Lux_LessThan::SetReference(int i, RefTargetHandle rtarg)
{
...
RefResult Lux_LessThan::NotifyRefChanged(const Interval& /*changeInt*/, RefTargetHandle hTarget, PartID& /*partID*/, RefMessage message, BOOL /*propagate*/)
{
	switch (message)
	{
...
RefTargetHandle Lux_LessThan::Clone(RemapDir &remap)
{
	Lux_LessThan *mnew = new Lux_LessThan();
...
Animatable* Lux_LessThan::SubAnim(int i)
{
...
TSTR Lux_LessThan::SubAnimName(int i, bool localized)
{
...
IOResult Lux_LessThan::Save(ISave* /*isave*/)
{
	//TODO: Add code to allow plug-in to save its data
	return IO_OK;
}

IOResult Lux_LessThan::Load(ILoad* /*iload*/)
{
	//TODO: Add code to allow plug-in to load its data
	return IO_OK;
}

AColor Lux_LessThan::EvalColor(ShadeContext& /*sc*/)
{
...
float Lux_LessThan::EvalMono(ShadeContext& sc)
{
	//TODO: Evaluate the map for a "mono" channel
	return Intens(EvalColor(sc));
}

Point3 Lux_LessThan::EvalNormalPerturb(ShadeContext& /*sc*/)
{
	//TODO: Return the perturbation to apply to a normal for bump mapping
	return Point3(0, 0, 0);
}

ULONG Lux_LessThan::LocalRequirements(int subMtlNum)
{
	//TODO: Specify various requirements for the material
	return uvGen->Requirements(subMtlNum);
}

void Lux_LessThan::ActivateTexDisplay(BOOL /*onoff*/)
{
	//TODO: Implement this only if SupportTexDisplay() returns TRUE
}

DWORD_PTR Lux_LessThan::GetActiveTexHandle(TimeValue /*t*/, TexHandleMaker& /*maker*/)
{
