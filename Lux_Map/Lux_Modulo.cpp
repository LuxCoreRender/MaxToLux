...
#define LUX_MODULO_CLASS_ID	Class_ID(0x7d03463, 0x4f2754d1)
...
class Lux_ModuloSampler : public MapSampler
{
private:
	Lux_Modulo* mTexture;
public:
	Lux_ModuloSampler() : mTexture(nullptr) { }
	Lux_ModuloSampler(Lux_Modulo *c) { mTexture = c; }
	~Lux_ModuloSampler() { }

	void   Set(Lux_Modulo *c) { mTexture = c; }
...
class Lux_Modulo : public Texmap {
public:
	...
	//From Animatable
	virtual Class_ID  ClassID() { return LUX_MODULO_CLASS_ID; }
	virtual SClass_ID SuperClassID() { return TEXMAP_CLASS_ID; }
	virtual void GetClassName(TSTR& s, bool localized) { s = GetString(IDS_CLASS_MODULO); }
...
class Lux_ModuloClassDesc : public ClassDesc2
...
public:
	...
	virtual void* Create(BOOL /*loading = FALSE*/) { return new Lux_Modulo(); }
	virtual const TCHAR *	ClassName() { return GetString(IDS_CLASS_MODULO); }
	const TCHAR*  NonLocalizedClassName() { return GetString(IDS_CLASS_MODULO); }
	virtual SClass_ID SuperClassID() { return TEXMAP_CLASS_ID; }
	virtual Class_ID ClassID() { return LUX_MODULO_CLASS_ID; }
	...
	virtual const TCHAR* InternalName() { return _T("Lux_Modulo"); }	// returns fixed parsable name (scripter-visible name)
	virtual HINSTANCE HInstance() { return hInstance; }					// returns owning module handle

};


ClassDesc2* GetLux_ModuloDesc() {
	static Lux_ModuloClassDesc Lux_ModuloDesc;
	return &Lux_ModuloDesc;
}


enum { Lux_Modulo_params };
...
static ParamBlockDesc2 Lux_Modulo_param_blk(Lux_Modulo_params, _T("params"), 0, GetLux_ModuloDesc(),
	P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	//rollout
	IDD_MODULO_PANEL, IDS_MODULO_PARAMS, 0, 0, NULL,
	// params
	value1, _T("modulo value 1"), TYPE_FLOAT, P_ANIMATABLE, IDS_CHECKER_TEXTURE1_COLOR,
	p_default, 1.0f,
	p_range, 0.001f, 1.0f,
	p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_MODULO_VALUE_1, IDC_MODULO_VALUE_1_SPIN, 0.001f,
	p_end,

	texture1_map, _T("modulo texture 1 map"), TYPE_TEXMAP, P_OWNERS_REF, IDS_CHECKER_TEXTURE1_MAP,
	p_refno, TEXTURE1_REF,
	p_subtexno, 0,
	p_ui, TYPE_TEXMAPBUTTON, IDC_MODULO_TEXTURE1_MAP,
	p_end,

	value2, _T("modulo value 2"), TYPE_FLOAT, P_ANIMATABLE, IDS_CHECKER_TEXTURE1_COLOR,
	p_default, 1.0f,
	p_range, 0.001f, 1.0f,
	p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_MODULO_VALUE_2, IDC_MODULO_VALUE_2_SPIN, 0.001f,
	p_end,

	texture2_map, _T("modulo texture 2 map"), TYPE_TEXMAP, P_OWNERS_REF, IDS_CHECKER_TEXTURE2_MAP,
	p_refno, TEXTURE2_REF,
	p_subtexno, 1,
	p_ui, TYPE_TEXMAPBUTTON, IDC_MODULO_TEXTURE2_MAP,
	p_end,
...
ParamDlg* Lux_Modulo::uvGenDlg;

//--- Lux_Modulo -------------------------------------------------------
Lux_Modulo::Lux_Modulo()
	: pblock(nullptr)
{
	for (int i = 0; i < NSUBTEX; i++)
		subtex[i] = nullptr;
	//TODO: Add all the initializing stuff
	GetLux_ModuloDesc()->MakeAutoParamBlocks(this);
	Reset();
}
...
void Lux_Modulo::Update(TimeValue /*t*/, Interval& /*valid*/)
{
	//TODO: Add code to evaluate anything prior to rendering
}

Interval Lux_Modulo::Validity(TimeValue /*t*/)
{
	//TODO: Update ivalid here
	return ivalid;
}

ParamDlg* Lux_Modulo::CreateParamDlg(HWND hwMtlEdit, IMtlParams* imp)
{
	IAutoMParamDlg* masterDlg = GetLux_ModuloDesc()->CreateParamDlgs(hwMtlEdit, imp, this);
...
BOOL Lux_Modulo::SetDlgThing(ParamDlg* dlg)
{
	if (dlg == uvGenDlg)
...
void Lux_Modulo::SetSubTexmap(int i, Texmap* m)
{
	ReplaceReference(i + 2, m);
	//TODO Store the 'i-th' sub-texmap managed by the texture
}

TSTR Lux_Modulo::GetSubTexmapSlotName(int i, bool localized)
{
...
//From ReferenceMaker
RefTargetHandle Lux_Modulo::GetReference(int i)
{
...
void Lux_Modulo::SetReference(int i, RefTargetHandle rtarg)
{
...
RefResult Lux_Modulo::NotifyRefChanged(const Interval& /*changeInt*/, RefTargetHandle hTarget, PartID& /*partID*/, RefMessage message, BOOL /*propagate*/)
{
	switch (message)
	{
...
RefTargetHandle Lux_Modulo::Clone(RemapDir &remap)
{
	Lux_Modulo *mnew = new Lux_Modulo();
...
Animatable* Lux_Modulo::SubAnim(int i)
{
...
TSTR Lux_Modulo::SubAnimName(int i, bool localized)
{
...
IOResult Lux_Modulo::Save(ISave* /*isave*/)
{
	//TODO: Add code to allow plug-in to save its data
	return IO_OK;
}

IOResult Lux_Modulo::Load(ILoad* /*iload*/)
{
	//TODO: Add code to allow plug-in to load its data
	return IO_OK;
}

AColor Lux_Modulo::EvalColor(ShadeContext& /*sc*/)
{
...
float Lux_Modulo::EvalMono(ShadeContext& sc)
{
	//TODO: Evaluate the map for a "mono" channel
	return Intens(EvalColor(sc));
}

Point3 Lux_Modulo::EvalNormalPerturb(ShadeContext& /*sc*/)
{
	//TODO: Return the perturbation to apply to a normal for bump mapping
	return Point3(0, 0, 0);
}

ULONG Lux_Modulo::LocalRequirements(int subMtlNum)
{
	//TODO: Specify various requirements for the material
	return uvGen->Requirements(subMtlNum);
}

void Lux_Modulo::ActivateTexDisplay(BOOL /*onoff*/)
{
	//TODO: Implement this only if SupportTexDisplay() returns TRUE
}

DWORD_PTR Lux_Modulo::GetActiveTexHandle(TimeValue /*t*/, TexHandleMaker& /*maker*/)
{
