...
#define LUX_ABS_CLASS_ID	Class_ID(0x7d03463, 0x4f8945d4)
...
class Lux_AbsSampler : public MapSampler
{
private:
	Lux_Abs* mTexture;
public:
	Lux_AbsSampler() : mTexture(nullptr) { }
	Lux_AbsSampler(Lux_Abs *c) { mTexture = c; }
	~Lux_AbsSampler() { }

	void   Set(Lux_Abs *c) { mTexture = c; }
...
public:
	...
	//From Animatable
	virtual Class_ID  ClassID() { return LUX_ABS_CLASS_ID; }
	virtual SClass_ID SuperClassID() { return TEXMAP_CLASS_ID; }
	virtual void GetClassName(TSTR& s, bool localized) { s = GetString(IDS_CLASS_ABS); }
...
class Lux_AbsClassDesc : public ClassDesc2
...
public:
	...
	virtual void* Create(BOOL /*loading = FALSE*/) { return new Lux_Abs(); }
	virtual const TCHAR *	ClassName() { return GetString(IDS_CLASS_ABS); }
	const TCHAR*  NonLocalizedClassName() { return GetString(IDS_CLASS_ABS); }
	virtual SClass_ID SuperClassID() { return TEXMAP_CLASS_ID; }
	virtual Class_ID ClassID() { return LUX_ABS_CLASS_ID; }
	...
	virtual const TCHAR* InternalName() { return _T("Lux_Abs"); }	// returns fixed parsable name (scripter-visible name)
	virtual HINSTANCE HInstance() { return hInstance; }					// returns owning module handle

};


ClassDesc2* GetLux_AbsDesc() {
	static Lux_AbsClassDesc Lux_AbsDesc;
	return &Lux_AbsDesc;
}
...
static ParamBlockDesc2 Lux_Abs_param_blk(Lux_Abs_params, _T("params"), 0, GetLux_AbsDesc(),
	P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
	//rollout
	IDD_ABS_PANEL, IDS_ABS_PARAMS, 0, 0, NULL,
	// params
	value1, _T("abs value 1"), TYPE_FLOAT, P_ANIMATABLE, IDS_ABS_VALUE,
	p_default, 1.0f,
	p_range, 0.001f, 1.0f,
	p_ui, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_ABS_VALUE, IDC_ABS_VALUE_SPIN, 0.001f,
	p_end,

	texture1_map, _T("abs texture 1 map"), TYPE_TEXMAP, P_OWNERS_REF, IDS_ABS_TEXTURE_MAP,
	p_refno, TEXTURE1_REF,
	p_subtexno, 0,
	p_ui, TYPE_TEXMAPBUTTON, IDC_ABS_TEXTURE_MAP,
	p_end,
	...
	: pblock(nullptr)
{
	...
	GetLux_AbsDesc()->MakeAutoParamBlocks(this);
	Reset();
}
...
void Lux_Abs::Update(TimeValue /*t*/, Interval& /*valid*/)
{
	//TODO: Add code to evaluate anything prior to rendering
}

Interval Lux_Abs::Validity(TimeValue /*t*/)
{
	//TODO: Update ivalid here
	return ivalid;
}

ParamDlg* Lux_Abs::CreateParamDlg(HWND hwMtlEdit, IMtlParams* imp)
{
	IAutoMParamDlg* masterDlg = GetLux_AbsDesc()->CreateParamDlgs(hwMtlEdit, imp, this);
...
BOOL Lux_Abs::SetDlgThing(ParamDlg* dlg)
{
	if (dlg == uvGenDlg)
...
void Lux_Abs::SetSubTexmap(int i, Texmap* m)
{
	ReplaceReference(i + 2, m);
	//TODO Store the 'i-th' sub-texmap managed by the texture
}

TSTR Lux_Abs::GetSubTexmapSlotName(int i, bool localized)
{
...
//From ReferenceMaker
RefTargetHandle Lux_Abs::GetReference(int i)
{
...
void Lux_Abs::SetReference(int i, RefTargetHandle rtarg)
{
...
RefResult Lux_Abs::NotifyRefChanged(const Interval& /*changeInt*/, RefTargetHandle hTarget, PartID& /*partID*/, RefMessage message, BOOL /*propagate*/)
{
	switch (message)
	{
...
RefTargetHandle Lux_Abs::Clone(RemapDir &remap)
{
	Lux_Abs *mnew = new Lux_Abs();
...
Animatable* Lux_Abs::SubAnim(int i)
{
...
TSTR Lux_Abs::SubAnimName(int i, bool localized)
{
...
IOResult Lux_Abs::Save(ISave* /*isave*/)
{
	//TODO: Add code to allow plug-in to save its data
	return IO_OK;
}

IOResult Lux_Abs::Load(ILoad* /*iload*/)
{
	//TODO: Add code to allow plug-in to load its data
	return IO_OK;
}

AColor Lux_Abs::EvalColor(ShadeContext& /*sc*/)
{
