...
class Lux_Metal : public Mtl {
public:
	Lux_Metal();
	Lux_Metal(BOOL loading);
	~Lux_Metal();
	...
	virtual void GetClassName(TSTR& s, bool localized) {s = GetString(IDS_CLASS_METAL);}
...
public:
	...
	virtual const TCHAR*  NonLocalizedClassName()	{ return GetString(IDS_CLASS_METAL); }
	virtual SClass_ID SuperClassID() 				{ return MATERIAL_CLASS_ID; }
	virtual Class_ID ClassID() 						{ return LUX_METAL_CLASS_ID; }
...
ClassDesc2* GetLux_MetalDesc() { 
	static Lux_MetalClassDesc Lux_MetalDesc;
	return &Lux_MetalDesc; 
}


enum { Lux_Metal_params };

enum { Metal_map, Common_Param, Light_emission };
...
static ParamBlockDesc2 Lux_Metal_param_blk (
	Lux_Metal_params, _T("params"),  0, GetLux_MetalDesc(),	P_AUTO_CONSTRUCT + P_AUTO_UI + P_MULTIMAP, PBLOCK_REF,
	3,
	//rollout
	Metal_map, IDD_METAL_PANEL, IDS_PARAMS, 0, 0, NULL,
	...
	preset2, _T("preset"), TYPE_INT, P_RESET_DEFAULT + P_ANIMATABLE, IDS_METAL_PRESET,
		...
		6, IDS_METAL_COMBOBOX_NONE, IDS_METAL_COMBOBOX_ALUMINIUM, IDS_METAL_COMBOBOX_SILVER, IDS_METAL_COMBOBOX_COPPER, IDS_METAL_COMBOBOX_GOLD,
...
void Lux_Metal::Reset()
{
	...
	GetLux_MetalDesc()->MakeAutoParamBlocks(this);
}



ParamDlg* Lux_Metal::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp)
{
	IAutoMParamDlg* masterDlg = GetLux_MetalDesc()->CreateParamDlgs(hwMtlEdit, imp, this);
	// TODO: Set param block user dialog if necessary
	return masterDlg;
	
}

BOOL Lux_Metal::SetDlgThing(ParamDlg* /*dlg*/)
{
	return FALSE;
}

Interval Lux_Metal::Validity(TimeValue t)
{
...
TSTR Lux_Metal::SubAnimName(int i, bool localized)
{
	...
	case REFMSG_CHANGE:
		...
		mapValid.SetEmpty();
			if (hTarget == pblock)
			{
				...
				Lux_Metal_param_blk.InvalidateUI(changing_param);
			}
		}
		break;
...
void Lux_Metal::SetSubMtl(int i, Mtl* m)
{
...
TSTR Lux_Metal::GetSubMtlSlotName(int i, bool localized)
{
...
Texmap* Lux_Metal::GetSubTexmap(int i)
{
...
TSTR Lux_Metal::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
		case 0:
...
TSTR Lux_Metal::GetSubTexmapTVName(int i)
{
...
IOResult Lux_Metal::Save(ISave* isave)
{
	IOResult res;
...
IOResult Lux_Metal::Load(ILoad* iload)
{
	IOResult res;
...
RefTargetHandle Lux_Metal::Clone(RemapDir &remap)
{
	Lux_Metal *mnew = new Lux_Metal(FALSE);
...
void Lux_Metal::NotifyChanged()
{
	NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
}

void Lux_Metal::Update(TimeValue t, Interval& valid)
{
...
void Lux_Metal::SetAmbient(Color /*c*/, TimeValue /*t*/) {}		
void Lux_Metal::SetDiffuse(Color /*c*/, TimeValue /*t*/) {}		
void Lux_Metal::SetSpecular(Color /*c*/, TimeValue /*t*/) {}
void Lux_Metal::SetShininess(float /*v*/, TimeValue /*t*/) {}

Color Lux_Metal::GetAmbient(int mtlNum, BOOL backFace)
{
	Point3 p;
...
Color Lux_Metal::GetDiffuse(int mtlNum, BOOL backFace)
{
	Point3 p;
	//TimeValue t; //Zero for first frame //GetCOREInterface()->GetTime() for every frame
...
float Lux_Metal::GetXParency(int mtlNum, BOOL backFace)
{
	float t = 0.0f;
...
float Lux_Metal::GetShininess(int mtlNum, BOOL backFace)
{
	float sh = 1.0f;
...
float Lux_Metal::GetShinStr(int mtlNum, BOOL backFace)
{
	return submtl[0] ? submtl[0]->GetShinStr(mtlNum,backFace): 0.0f;
}

float Lux_Metal::WireSize(int mtlNum, BOOL backFace)
{
	float wf = 0.0f;
...
void Lux_Metal::Shade(ShadeContext& sc)
{
...
float Lux_Metal::EvalDisplacement(ShadeContext& sc)
{
	Mtl* subMaterial = mapOn[0] ? submtl[0] : nullptr;
	return (subMaterial) ? subMaterial->EvalDisplacement(sc) : 0.0f;
}

Interval Lux_Metal::DisplacementValidity(TimeValue t)
{
	Mtl* subMaterial = mapOn[0] ? submtl[0] : nullptr;
