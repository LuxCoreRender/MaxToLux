...
class Lux_Carpaint : public Mtl {
public:
	Lux_Carpaint();
	Lux_Carpaint(BOOL loading);
	~Lux_Carpaint();
	...
	virtual void GetClassName(TSTR& s, bool localized) {s = GetString(IDS_CLASS_CARPAINT);}
...
class Lux_CarpaintClassDesc : public ClassDesc2 
...
public:
	...
	virtual const TCHAR*  NonLocalizedClassName()	{ return GetString(IDS_CLASS_CARPAINT); }
	virtual SClass_ID SuperClassID() 				{ return MATERIAL_CLASS_ID; }
	virtual Class_ID ClassID() 						{ return LUX_CARPAINT_CLASS_ID; }
...
ClassDesc2* GetLux_CarpaintDesc() { 
	static Lux_CarpaintClassDesc Lux_CarpaintDesc;
	return &Lux_CarpaintDesc; 
}


enum { Lux_Carpaint_params };

enum { Carpaint_map, Common_Param, Light_emission };
...
enum 
{
	...
	fresnel_layer3_map,
	roughness_layer1,
	roughness_layer1_map,
	roughness_layer2,
	roughness_layer2_map,
	roughness_layer3,
	roughness_layer3_map,
...
static ParamBlockDesc2 Lux_Carpaint_param_blk (
	...
	preset, _T("preset"), TYPE_INT, P_RESET_DEFAULT + P_ANIMATABLE, IDS_CARPAINT_PRESET,
		...
		p_vals, 1, 2, 3, 4, 5, 6, 7, 8,
		p_default, 1,
		p_tooltip, IDS_CARPAINT_COMBOBOX_INT,
		p_end,

	absorption, _T("absorption"), TYPE_RGBA, P_ANIMATABLE, IDS_CARPAINT_ABSORPTION,
		p_default, Color(0.0f, 0.0f, 0.0f),
		p_ui, Carpaint_map, TYPE_COLORSWATCH, IDC_CARPAINT_ABSORPTION,
		p_end,
	...
	depth, _T("depth"), TYPE_FLOAT, P_ANIMATABLE, IDS_CARPAINT_DEPTH,
		p_default, 0.0f,
		p_range, 0.0f, 9999.0f,
		p_ui, Carpaint_map, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_CARPAINT_DEPTH, IDC_CARPAINT_DEPTH_SPIN, 0.1f,
		p_end,
	...
	diffuse, _T("diffuse"), TYPE_RGBA, P_ANIMATABLE, IDS_CARPAINT_DIFFUSE,
		p_default, Color(0.0f, 0.0f, 0.0f),
		p_ui, Carpaint_map, TYPE_COLORSWATCH, IDC_CARPAINT_DIFFUSE,
		p_end,
	...
	specular_layer1, _T("specular layer 1"), TYPE_RGBA, P_ANIMATABLE, IDS_CARPAINT_SPECULAR_LAYER_1,
		p_default, Color(0.0f, 0.0f, 0.0f),
		p_ui, Carpaint_map, TYPE_COLORSWATCH, IDC_CARPAINT_SPECULAR_LAYER_1,
		p_end,
	...
	specular_layer2, _T("specular layer 2"), TYPE_RGBA, P_ANIMATABLE, IDS_CARPAINT_SPECULAR_LAYER_2,
		p_default, Color(0.0f, 0.0f, 0.0f),
		p_ui, Carpaint_map, TYPE_COLORSWATCH, IDC_CARPAINT_SPECULAR_LAYER_2,
		p_end,
	...
	specular_layer3, _T("specular layer 3"), TYPE_RGBA, P_ANIMATABLE, IDS_CARPAINT_SPECULAR_LAYER_3,
		p_default, Color(0.0f, 0.0f, 0.0f),
		p_ui, Carpaint_map, TYPE_COLORSWATCH, IDC_CARPAINT_SPECULAR_LAYER_3,
		p_end,
	...
	fresnel_layer1, _T("fresnel layer 1"), TYPE_RGBA, P_ANIMATABLE, IDS_CARPAINT_FRESNEL_LAYER_1,
		p_default, Color(0.0f, 0.0f, 0.0f),
		p_ui, Carpaint_map, TYPE_COLORSWATCH, IDC_CARPAINT_ROUGHNESS_LAYER_1,
		p_end,
	...
	fresnel_layer2, _T("fresnel layer 2"), TYPE_RGBA, P_ANIMATABLE, IDS_CARPAINT_FRESNEL_LAYER_2,
		p_default, Color(0.0f, 0.0f, 0.0f),
		p_ui, Carpaint_map, TYPE_COLORSWATCH, IDC_CARPAINT_ROUGHNESS_LAYER_2,
		p_end,
	...
	fresnel_layer3, _T("fresnel layer 3"), TYPE_RGBA, P_ANIMATABLE, IDS_CARPAINT_FRESNEL_LAYER_3,
		p_default, Color(0.0f, 0.0f, 0.0f),
		p_ui, Carpaint_map, TYPE_COLORSWATCH, IDC_CARPAINT_ROUGHNESS_LAYER_3,
		p_end,
	...
	roughness_layer1, _T("roughness layer 1"), TYPE_RGBA, P_ANIMATABLE, IDS_CARPAINT_ROUGHNESS_LAYER_1,
		p_default, Color(0.0f, 0.0f, 0.0f),
		p_ui, Carpaint_map, TYPE_COLORSWATCH, IDC_CARPAINT_FRESNEL_LAYER_1,
		p_end,
	...
	roughness_layer2, _T("roughness layer 2"), TYPE_RGBA, P_ANIMATABLE, IDS_CARPAINT_ROUGHNESS_LAYER_2,
		p_default, Color(0.0f, 0.0f, 0.0f),
		p_ui, Carpaint_map, TYPE_COLORSWATCH, IDC_CARPAINT_FRESNEL_LAYER_2,
		p_end,
	...
	roughness_layer3, _T("roughness layer 3"), TYPE_RGBA, P_ANIMATABLE, IDS_CARPAINT_ROUGHNESS_LAYER_3,
		p_default, Color(0.0f, 0.0f, 0.0f),
		p_ui, Carpaint_map, TYPE_COLORSWATCH, IDC_CARPAINT_FRESNEL_LAYER_3,
		p_end,
...
Lux_Carpaint::Lux_Carpaint(BOOL loading)
	: pblock(nullptr)
{
	for (int i = 0; i < NUM_SUBMATERIALS; i++)
	{
...
void Lux_Carpaint::Reset()
{
	...
	GetLux_CarpaintDesc()->MakeAutoParamBlocks(this);
}



ParamDlg* Lux_Carpaint::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp)
{
	IAutoMParamDlg* masterDlg = GetLux_CarpaintDesc()->CreateParamDlgs(hwMtlEdit, imp, this);
	// TODO: Set param block user dialog if necessary
	return masterDlg;
	
}

BOOL Lux_Carpaint::SetDlgThing(ParamDlg* /*dlg*/)
{
	return FALSE;
}

Interval Lux_Carpaint::Validity(TimeValue t)
{
...
RefTargetHandle Lux_Carpaint::GetReference(int i)
{
	/*switch (i)
	{
...
void Lux_Carpaint::SetReference(int i, RefTargetHandle rtarg)
{
...
TSTR Lux_Carpaint::SubAnimName(int i, bool localized)
{
...
RefResult Lux_Carpaint::NotifyRefChanged(const Interval& /*changeInt*/, RefTargetHandle hTarget, 
	...
	case REFMSG_CHANGE:
		{
			...
			if (hTarget == pblock)
			{
				...
				Lux_Carpaint_param_blk.InvalidateUI(changing_param);
			}
		}
		break;
...
Mtl* Lux_Carpaint::GetSubMtl(int i)
{
...
TSTR Lux_Carpaint::GetSubMtlSlotName(int i, bool localized)
{
...
TSTR Lux_Carpaint::GetSubMtlTVName(int i)
{
...
Texmap* Lux_Carpaint::GetSubTexmap(int i)
{
...
TSTR Lux_Carpaint::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
		case 0:
		...
		case 3:
			return _T("specular 1 map");
		case 4:
			return _T("specular 2 map");
		case 5:
			return _T("specular 3 map");
		case 6:
			return _T("roughness 1 map");
		case 7:
			return _T("roughness 2 map");
		case 8:
			return _T("roughness 3 map");
		case 9:
			return _T("fresnel 1 map");
		case 10:
			return _T("fresnel 2 map");
		case 11:
			return _T("fresnel 3 map");
		case 12:
			return _T("Bump map");
		case 13:
...
TSTR Lux_Carpaint::GetSubTexmapTVName(int i)
{
...
IOResult Lux_Carpaint::Save(ISave* isave)
{
	IOResult res;
...
IOResult Lux_Carpaint::Load(ILoad* iload)
{
	IOResult res;
...
RefTargetHandle Lux_Carpaint::Clone(RemapDir &remap)
{
	Lux_Carpaint *mnew = new Lux_Carpaint(FALSE);
...
void Lux_Carpaint::NotifyChanged()
{
	NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
}

void Lux_Carpaint::Update(TimeValue t, Interval& valid)
{
...
void Lux_Carpaint::SetAmbient(Color /*c*/, TimeValue /*t*/) {}		
void Lux_Carpaint::SetDiffuse(Color /*c*/, TimeValue /*t*/) {}		
void Lux_Carpaint::SetSpecular(Color /*c*/, TimeValue /*t*/) {}
void Lux_Carpaint::SetShininess(float /*v*/, TimeValue /*t*/) {}

Color Lux_Carpaint::GetAmbient(int mtlNum, BOOL backFace)
{
	Point3 p;
	//TimeValue t; //Zero for first frame //GetCOREInterface()->GetTime() for every frame
	pblock->GetValue(absorption, GetCOREInterface()->GetTime(), p, ivalid);
	return submtl[0] ? submtl[0]->GetAmbient(mtlNum, backFace) : Color(p.x, p.y, p.z);//Bound(Color(p.x, p.y, p.z));
}

Color Lux_Carpaint::GetDiffuse(int mtlNum, BOOL backFace)
{
	Point3 p;
	//TimeValue t; //Zero for first frame //GetCOREInterface()->GetTime() for every frame
	pblock->GetValue(absorption, 0, p, ivalid);
	return submtl[0] ? submtl[0]->GetDiffuse(mtlNum, backFace) : Color(p.x, p.y, p.z);
}

Color Lux_Carpaint::GetSpecular(int mtlNum, BOOL backFace)
{
	Point3 p;
	pblock->GetValue(absorption, 0, p, ivalid);
...
float Lux_Carpaint::GetXParency(int mtlNum, BOOL backFace)
{
	float t = 0.0f;
...
float Lux_Carpaint::GetShininess(int mtlNum, BOOL backFace)
{
	float sh = 1.0f;
...
float Lux_Carpaint::GetShinStr(int mtlNum, BOOL backFace)
{
	return submtl[0] ? submtl[0]->GetShinStr(mtlNum,backFace): 0.0f;
}

float Lux_Carpaint::WireSize(int mtlNum, BOOL backFace)
{
	float wf = 0.0f;
...
void Lux_Carpaint::Shade(ShadeContext& sc)
{
...
float Lux_Carpaint::EvalDisplacement(ShadeContext& sc)
{
	Mtl* subMaterial = mapOn[0] ? submtl[0] : nullptr;
	return (subMaterial) ? subMaterial->EvalDisplacement(sc) : 0.0f;
}

Interval Lux_Carpaint::DisplacementValidity(TimeValue t)
{
	Mtl* subMaterial = mapOn[0] ? submtl[0] : nullptr;
