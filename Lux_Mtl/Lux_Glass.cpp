...
#define LUX_GLASS_CLASS_ID	Class_ID(0x417b35ee, 0x15660db8)
...
static int seed = rand() % 8400 + 7400;

class Lux_Glass : public Mtl {
public:
	Lux_Glass();
	Lux_Glass(BOOL loading);
	~Lux_Glass();
	...
	// From Animatable
	virtual Class_ID ClassID() {return LUX_GLASS_CLASS_ID;}
	virtual SClass_ID SuperClassID() { return MATERIAL_CLASS_ID; }
	virtual void GetClassName(TSTR& s, bool localized) {s = GetString(IDS_CLASS_GLASS);}
...
class Lux_GlassClassDesc : public ClassDesc2 
...
public:
	...
	virtual void* Create(BOOL loading = FALSE) 		{ return new Lux_Glass(loading); }
	virtual const TCHAR *	ClassName() 			{ return GetString(IDS_CLASS_GLASS); }
	virtual const TCHAR*  NonLocalizedClassName()	{ return GetString(IDS_CLASS_GLASS); }
	virtual SClass_ID SuperClassID() 				{ return MATERIAL_CLASS_ID; }
	virtual Class_ID ClassID() 						{ return LUX_GLASS_CLASS_ID; }
	...
	virtual const TCHAR* InternalName() 			{ return _T("Lux_Glass"); }	// returns fixed parsable name (scripter-visible name)
...
ClassDesc2* GetLux_GlassDesc() { 
	static Lux_GlassClassDesc Lux_GlassDesc;
	return &Lux_GlassDesc; 
}

enum { Lux_Glass_params };

enum { Glass_map, Common_Param, Light_emission };
...
static ParamBlockDesc2 Lux_Glass_param_blk (
	Lux_Glass_params, _T("params"),  0, GetLux_GlassDesc(),	P_AUTO_CONSTRUCT + P_AUTO_UI + P_MULTIMAP, PBLOCK_REF,
	3,
	//rollout
	Glass_map,			IDD_GLASS_PANEL,	IDS_PARAMS,			0,	0,	NULL,
	Common_Param,		IDD_COMMON_PANEL,	IDS_COMMON_PARAMS,	0,	0,	NULL,
	Light_emission,		IDD_LIGHT_PANEL,	IDS_LIGHT_PARAMS,	0,	0,	NULL,

	// params
	reflection_color, _T("Reflextion color"), TYPE_RGBA, P_ANIMATABLE, IDS_GlASS_REFLECTION_COLOR,
		p_default, Color(1.0f, 1.0f, 1.0f),
		p_ui, Glass_map, TYPE_COLORSWATCH, IDC_GLASS_REFLECTION_COLOR,
		p_end,

	reflection_map, _T("Reflextion map"), TYPE_TEXMAP, P_OWNERS_REF, IDS_GlASS_REFLECTION_MAP,
		p_refno, 2,
		p_subtexno, 0,
		p_ui, Glass_map, TYPE_TEXMAPBUTTON, IDC_GLASS_REFLECTION_MAP,
		p_end,

	transmit_color, _T("Transmit color"), TYPE_RGBA, P_ANIMATABLE, IDS_GlASS_TRANSMIT_COLOR,
		p_default, Color(1.0f, 1.0f, 1.0f),
		p_ui, Glass_map, TYPE_COLORSWATCH, IDC_GLASS_TRANSMIT_COLOR,
		p_end,

	transmit_map, _T("Transmit map"), TYPE_TEXMAP, P_OWNERS_REF, IDS_GlASS_TRANSMIT_MAP,
		p_refno, 3,
		p_subtexno, 1,
		p_ui, Glass_map, TYPE_TEXMAPBUTTON, IDC_GLASS_TRANSMIT_MAP,
		p_end,

	interiorior, _T("interiorior"), TYPE_FLOAT, P_ANIMATABLE, IDS_GLASS_INTERIOR_SPIN,
		p_default, 1.5f,
		p_range, 0.0f, 999.0f,
		p_ui, Glass_map, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_GLASS_INTERIORIOR, IDC_GLASS_INTERIORIOR_SPIN, 0.1f,
		p_end,

	interiorior_map, _T("interiorior map"), TYPE_TEXMAP, P_OWNERS_REF, IDS_GlASS_INTERIORIOR_MAP,
		p_refno, 4,
		p_subtexno, 2,
	...
	exteriorior, _T("exteriorior"), TYPE_FLOAT, P_ANIMATABLE, IDS_GLASS_EXTERIOR_SPIN,
		p_default, 1.0f,
		p_range, 0.0f, 999.0f,
		p_ui, Glass_map, TYPE_SPINNER, EDITTYPE_FLOAT, IDC_GLASS_EXTERIORIOR, IDC_GLASS_EXTERIORIOR_SPIN, 0.1f,
		p_end,

	exteriorior_map, _T("exteriorior map"), TYPE_TEXMAP, P_OWNERS_REF, IDS_GlASS_EXTERIORIOR_MAP,
		p_refno, 5,
		p_subtexno, 3,
...
Lux_Glass::Lux_Glass(BOOL loading)
	: pblock(nullptr)
{
	for (int i = 0; i < NUM_SUBMATERIALS; i++)
	{
...
void Lux_Glass::Reset()
{
	...
	GetLux_GlassDesc()->MakeAutoParamBlocks(this);
}



ParamDlg* Lux_Glass::CreateParamDlg(HWND hwMtlEdit, IMtlParams *imp)
{
	IAutoMParamDlg* masterDlg = GetLux_GlassDesc()->CreateParamDlgs(hwMtlEdit, imp, this);
	// TODO: Set param block user dialog if necessary
	return masterDlg;
	
}

BOOL Lux_Glass::SetDlgThing(ParamDlg* /*dlg*/)
{
	return FALSE;
}

Interval Lux_Glass::Validity(TimeValue t)
{
...
RefTargetHandle Lux_Glass::GetReference(int i)
{
	/*switch (i)
	{
...
void Lux_Glass::SetReference(int i, RefTargetHandle rtarg)
{
...
TSTR Lux_Glass::SubAnimName(int i, bool localized)
{
...
Animatable* Lux_Glass::SubAnim(int i)
{
	/*switch (i)
	{
...
RefResult Lux_Glass::NotifyRefChanged(const Interval& /*changeInt*/, RefTargetHandle hTarget, 
	...
	case REFMSG_CHANGE:
		...
		mapValid.SetEmpty();
			if (hTarget == pblock)
			{
				...
				Lux_Glass_param_blk.InvalidateUI(changing_param);
			}
		}
		break;
...
void Lux_Glass::SetSubMtl(int i, Mtl* m)
{
	//mprintf(_T("\n SetSubMtl Nubmer is : %i \n"), i);
	ReplaceReference(i , m);
}

TSTR Lux_Glass::GetSubMtlSlotName(int i, bool localized)
{
...
TSTR Lux_Glass::GetSubMtlTVName(int i)
{
...
Texmap* Lux_Glass::GetSubTexmap(int i)
{
...
void Lux_Glass::SetSubTexmap(int i, Texmap* tx)
{
	...
	{
		case 0:
			Lux_Glass_param_blk.InvalidateUI(reflection_map);
			mapValid.SetEmpty();
			break;
		case 1:
			Lux_Glass_param_blk.InvalidateUI(transmit_map);
			mapValid.SetEmpty();
			break;
		case 2:
			Lux_Glass_param_blk.InvalidateUI(interiorior_map);
			mapValid.SetEmpty();
			break;
		case 3:
			Lux_Glass_param_blk.InvalidateUI(exteriorior_map);
			mapValid.SetEmpty();
			break;
		case 4:
			Lux_Glass_param_blk.InvalidateUI(bump_map);
			mapValid.SetEmpty();
			break;
		case 5:
			Lux_Glass_param_blk.InvalidateUI(normal_map);
			mapValid.SetEmpty();
			break;
		case 6:
			Lux_Glass_param_blk.InvalidateUI(interior_map);
			mapValid.SetEmpty();
			break;
		case 7:
			Lux_Glass_param_blk.InvalidateUI(exterior_map);
			mapValid.SetEmpty();
			break;
		case 8:
			Lux_Glass_param_blk.InvalidateUI(emission_map);
			mapValid.SetEmpty();
			break;
		case 9:
			Lux_Glass_param_blk.InvalidateUI(emission_mapfile);
			mapValid.SetEmpty();
			break;
		case 10:
			Lux_Glass_param_blk.InvalidateUI(emission_iesfile);
			mapValid.SetEmpty();
			break;
	}
}

TSTR Lux_Glass::GetSubTexmapSlotName(int i, bool localized)
{
	switch (i)
	{
		case 0:
...
TSTR Lux_Glass::GetSubTexmapTVName(int i)
{
...
IOResult Lux_Glass::Save(ISave* isave)
{
	IOResult res;
...
IOResult Lux_Glass::Load(ILoad* iload)
{
	IOResult res;
...
RefTargetHandle Lux_Glass::Clone(RemapDir &remap)
{
	Lux_Glass *mnew = new Lux_Glass(FALSE);
...
void Lux_Glass::NotifyChanged()
{
	NotifyDependents(FOREVER, PART_ALL, REFMSG_CHANGE);
}

void Lux_Glass::Update(TimeValue t, Interval& valid)
{
...
void Lux_Glass::SetAmbient(Color /*c*/, TimeValue /*t*/) {}		
void Lux_Glass::SetDiffuse(Color /*c*/, TimeValue /*t*/) {}		
void Lux_Glass::SetSpecular(Color /*c*/, TimeValue /*t*/) {}
void Lux_Glass::SetShininess(float /*v*/, TimeValue /*t*/) {}

Color Lux_Glass::GetAmbient(int mtlNum, BOOL backFace)
{
	Point3 p;
...
Color Lux_Glass::GetDiffuse(int mtlNum, BOOL backFace)
{
	Point3 p;
	//TimeValue t; //Zero for first frame //GetCOREInterface()->GetTime() for every frame
...
Color Lux_Glass::GetSpecular(int mtlNum, BOOL backFace)
{
	Point3 p;
	pblock->GetValue(reflection_color, 0, p, ivalid);
...
float Lux_Glass::GetXParency(int mtlNum, BOOL backFace)
{
	float t = 0.0f;
...
float Lux_Glass::GetShininess(int mtlNum, BOOL backFace)
{
	float sh = 1.0f;
...
float Lux_Glass::GetShinStr(int mtlNum, BOOL backFace)
{
	return submtl[0] ? submtl[0]->GetShinStr(mtlNum,backFace): 0.0f;
}

float Lux_Glass::WireSize(int mtlNum, BOOL backFace)
{
	float wf = 0.0f;
...
void Lux_Glass::Shade(ShadeContext& sc)
{
...
float Lux_Glass::EvalDisplacement(ShadeContext& sc)
{
	Mtl* subMaterial = mapOn[0] ? submtl[0] : nullptr;
	return (subMaterial) ? subMaterial->EvalDisplacement(sc) : 0.0f;
}

Interval Lux_Glass::DisplacementValidity(TimeValue t)
{
	Mtl* subMaterial = mapOn[0] ? submtl[0] : nullptr;
